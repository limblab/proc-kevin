...
%
%       .plotCentralCross Controls whether the central cross is plotted.  The default is 'true'
%       .centralCrossSize The default is 7.5
%       .arrowSize        The default is 5
%       .arrowLineWidth   The default is 0.5
...
%       .useVerticalAxis  whether vertical axis should be plotted.  Default is 'true'
%       .useHorizontalAxis  whether horizontal axis should be plotted.  Default is 'true'
%       .useAxisLabels    whether to label the axes.  Default is 'true'
%       .useLabel         whether to label with dataset. Default is 'true'
%       .useTickLabels    whether to label ticks. Default is 'true'
%       .planMarkerSize   size of the plan dot.  Default is 6.
%       .planLineWidth    The default is 0.5
...
%
function [colorStruct, haxP, vaxP, handles] = phaseSpace(Projection, Summary, params)
...
arrowLineWidth = 0.5;
if exist('params', 'var') && isfield(params,'arrowLineWidth')
    arrowLineWidth = params.arrowLineWidth;
end
...
centralCrossSize = 7.5;
if exist('params', 'var') && isfield(params,'centralCrossSize')
    centralCrossSize = params.centralCrossSize;
end

plotCentralCross = true;
if exist('params', 'var') && isfield(params,'plotCentralCross')
    plotCentralCross = params.plotCentralCross;
end
...
if exist('params', 'var') && isfield(params,'useHorizontalAxis')
    useHorizontalAxis = params.useHorizontalAxis;
end

useVerticalAxis = true;
if exist('params', 'var') && isfield(params,'useVerticalAxis')
    useVerticalAxis = params.useVerticalAxis;
end

useAxisLabels = true;
if exist('params', 'var') && isfield(params,'useAxisLabels')
    useAxisLabels = params.useAxisLabels;
end
...
useTickLabels = true;
if exist('params', 'var') && isfield(params,'useTickLabels')
    useTickLabels = params.useTickLabels;
end
...
planLineWidth = 0.5;
if exist('params', 'var') && isfield(params,'planLineWidth')
    planLineWidth = params.planLineWidth;
end
...
if exist('params', 'var') && isfield(params,'colors2use')
    colors2use = params.colors2use;
end
...
for pindex = 1:length(planes2plot)
    ...
    if exist('params', 'var') && isfield(params,'colors')
        lineColor = params(pindex).colors;
        arrowFaceColor = params(pindex).colors;
        planMarkerColor = params(pindex).colors;
        disp('hi');
    end
    ...
    if reusePlot == 0, jPCA_with_CP_mods.fromMarksLibraries.blankFigure(axLim); else cla; end

    % first deal with the ellipse for the plan variance (we want this under the rest of the data)
    if plotPlanEllipse, jPCA_with_CP_mods.fromMarksLibraries.circle([0 0], ellipseRadii, 0.6*[1 1 1], 1); end
    ...
    for c = 1:numConds
        ...
        if ismember(c,conds2plot)
            htmp = plot(P1, P2, 'color', lineColor{c}, 'lineWidth', lineWidth);
            handles.trajectory( c ) = htmp;

            if planMarkerSize>0
                htmp = plot(P1(1), P2(1), 'ko', 'markerSize', planMarkerSize, 'markerFaceColor', planMarkerColor{c});
                htmp.LineWidth = planLineWidth;
                handles.planMarker( c ) = htmp;
            end
            ...
            if isempty(arrowMinVel) || vel > arrowMinVel
                ...
                htmp = jPCA_with_CP_mods.fromMarksLibraries.arrowMMC(...
                    penultimatePoint, lastPoint, [], aSize, axLim, arrowFaceColor{c}, arrowEdgeColor);
                htmp.LineWidth = arrowLineWidth;
                handles.arrow( c ) = htmp;
            else
                htmp = plot(lastPoint(1), lastPoint(2), 'ko', 'markerSi', arrowSize, 'markerFac', arrowFaceColor{c}, 'markerEdge', arrowEdgeColor);
                handles.arrow( c ) = htmp;
            end
        end
    ...
    if plotCentralCross
        plot(0,0,'b+', 'markerSi', centralCrossSize);  % plot a central cross
    end
    ...
    if crossCondMean && length(Summary.crossCondMean) > 1
        ...
        aSize = arrowSize + arrowGain * vel;  % if asked (e.g. for movies) arrow size may grow with vel
        jPCA_with_CP_mods.fromMarksLibraries.arrowMMC(penultimatePoint, lastPoint, [], aSize, axLim, meanColor, arrowEdgeColor);
    ...
    if useAxes
        ...
        if ~useTickLabels
            axisParams.tickLabels=repmat( {''}, 1, numel(axisParams.tickLocations) );
            axisParams.tickLabelLocations = axisParams.tickLocations;
        end
        if useHorizontalAxis
            haxP = jPCA_with_CP_mods.fromMarksLibraries.AxisMMC(-axisLength, axisLength, axisParams);
        else
            haxP.axisLabelOffset = farthestDown + extraSeparation;
        end
        haxP.farthestDown = farthestDown;
        ...
        if useVerticalAxis
            vaxP = jPCA_with_CP_mods.fromMarksLibraries.AxisMMC(-axisLength, axisLength, axisParams);
        end
        vaxP.farthestLeft = farthestLeft;
    end
