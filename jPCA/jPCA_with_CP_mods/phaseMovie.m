...
%       .arrowSize    Default is 3.3. phaseSpace() default is 5
%       .tail         If specified and not empty, a tail of this length (in ms) will be produced, instead of the whole trajectory. 
%       .colorsToUse      which colors to use. each row is a condition, each coloumn is r,g,b. values range from 0 to 1


function MV = phaseMovie(Projection, Summary, movieParams, outputFile, frameRate)
...
if exist('movieParams','var') && isfield(movieParams,'colorsToUse')
    frameParams.colorsToUse = movieParams.colorsToUse;
end
...
stationaryPadEnd = 18;  % extra frames at the end of stationary image.
...
if exist('movieParams', 'var') && isfield(movieParams,'arrowSize')
    frameParams.arrowSize = movieParams.arrowSize;
end
...
%videoObj = VideoWriter(outputFile);
%set(videoObj, 'FrameRate', frameRate);
%open(videoObj);
...
if nargout > 0  && usePads == 1
    ...
    for i = 1:stationaryPadStart
        ...
        jPCA_with_CP_mods.phaseSpace(Projection, Summary, frameParams);
        drawnow;
        frameParams.reusePlot = 1;  % after the first frame, always reuse

        frame.cdata = print('-RGBImage');
        frame.colormap = [];
        disp( sprintf( 'writing frame %g / %g', i, stationaryPadStart ) );
        %writeVideo(videoObj, frame);

        if nargout > 0
            thisFrame=jPCA_with_CP_mods.getScreenshotnapshot(gcf);
            %MV(fi) = getframe(gca, pixelsToGet);
            MV(fi).cdata = thisFrame;
            MV(fi).colormap = [];
            fi=fi+1;
        end
    end
...
for ti = 2:length(times2plot)
    ...
    %frame.cdata = print('-RGBImage');
    %frame.colormap = [];
    disp( sprintf( 'writing frame %g / %g', ti, length(times2plot) ) );
    %writeVideo(videoObj, frame);

    if nargout > 0
        thisFrame=jPCA_with_CP_mods.getScreenshot(gcf);
        %MV(fi) = getframe(gca, pixelsToGet);
        MV(fi).cdata = thisFrame;
        MV(fi).colormap = [];
        fi=fi+1;
...
if nargout > 0  && usePads == 1
    ...
    for i = 1:stationaryPadEnd
        ...
        %frame.cdata = print('-RGBImage');
        %frame.colormap = [];
        %disp( sprintf( 'writing frame %g / %g', i, stationaryPadEnd ) );
        %writeVideo(videoObj, frame);
