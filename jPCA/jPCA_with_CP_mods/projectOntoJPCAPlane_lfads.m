function Projections = projectOntoJPCAPlane_lfads(originalData,analyzeTimes,params,Summary,newData)
...
meanFReachNeuron = mean(smallA);  % this will be kept for use by future attempts to project onto the PC space
...
Data = newData;
for c = 1:numel(Data)
    Data(c).A = [Data(c).rates]';
    Data(c).times = Data(c).timeVecMs
