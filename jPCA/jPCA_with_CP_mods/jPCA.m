...
%   data you must do this in the same order (using the ORIGINAL normFactors and mean firing rates.
...
% Mskew = jPCA_with_CP_mods.skewSymRegress(dState,preState)';  % this is the best Mskew for the same equation
Mskew = skewSymRegress(dState,preState)'; % I'm really not sure what this dot notation is...
...
%     jPCA_with_CP_mods.fromMarksLibraries.circle([0 0], 2*planVars.^0.5, 0.6*[1 1 1], 1); hold on;
...
%         jPCA_with_CP_mods.fromMarksLibraries.arrowMMC(penultimatePoint, lastPoint, [], arrowSize, axLim);
...
function phaseData = getPhase(Proj, whichPair)
    ...
    for c=1:numConds
        ...
        % between -pi and pi
%         phaseData(c).phaseDiff = jPCA_with_CP_mods.fromMarksLibraries.minusPi2Pi(phaseData(c).phaseOfDelta - phaseData(c).phase); %#ok<AGROW>
...
%     circMn = jPCA_with_CP_mods.CircStat2010d.circ_mean([phaseData.phaseDiff]', [phaseData.radius]');
%     resultantVect = jPCA_with_CP_mods.CircStat2010d.circ_r([phaseData.phaseDiff]', [phaseData.radius]');
